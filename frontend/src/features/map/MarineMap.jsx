...
import { Compass, Anchor, AlertTriangle, ShieldCheck, Fish, Layers } from 'lucide-react';
...
export default function MarineMap({
  ...
  const getStyleForLayer = (feature) => {
    ...
    if (layerType === 'MPA') {
      return { color: '#10b981', weight: 2, fillOpacity: 0.25, fillColor: '#059669' };
    }
    if (layerType === 'RESTRICTED') {
      return { color: '#f43f5e', weight: 2, fillOpacity: 0.3, fillColor: '#e11d48', dashArray: '4, 4' };
    }
    if (layerType === 'HAZARD') {
      return { color: '#f59e0b', weight: 2, fillOpacity: 0.35, fillColor: '#d97706' };
    }
    if (layerType === 'PFZ') {
      return { color: '#06b6d4', weight: 2, fillOpacity: 0.3, fillColor: '#0891b2' };
    }
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden border border-slate-800" style={{ height }}>
      <MapContainer
        center={center}
        zoom={compact ? 9 : 8}
