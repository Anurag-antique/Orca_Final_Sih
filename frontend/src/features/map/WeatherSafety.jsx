...
const LABELS = { LOW: 'Lower weather risk', MODERATE: 'Use caution in these conditions', HIGH: 'Critical conditions — avoid departure', CRITICAL: 'Critical conditions — avoid departure' };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) onReading({ point, weather, ocean, context, risk: result.data,
          isFallback: isMock(weather) || isMock(ocean) || isMock(result), color: COLORS[result.data.riskLevel] });
      } catch (error) {
  ...
  return <section className="rounded-2xl border border-slate-700 bg-slate-900 p-5 space-y-3 text-sm" aria-label="Weather safety" aria-live="polite">
    ...
    {reading?.risk && <>
      <div className="rounded-lg border-2 p-3" style={{ borderColor: reading.color }}>
        ...
        <p className={`mt-2 rounded px-2 py-1 text-xs ${reading.isFallback ? 'bg-amber-950 text-amber-200' : 'bg-sky-950 text-sky-200'}`}>
          {reading.isFallback ? 'Some readings are backup estimates — check again before leaving.' : 'Live readings from the weather and ocean services.'}
        </p>
      </div>
      <details className="rounded-lg border border-slate-700 bg-slate-950/70 p-3 text-xs text-slate-300">
        ...
        <p className="mt-1 text-slate-400">This is a weather and ocean estimate, not legal navigation clearance. Check official notices before departure. SST and chlorophyll describe fishing conditions and do not decide vessel safety.</p>
      </details>
    </>}
