  ...
  const handlePlanRoute = async () => {
    ...
    try {
      const res = await routeService.planRoute(
        selectedOrigin,
        selectedDestination,
        { typeKey: vesselType, name: vesselType === 'small_motorized' ? 'Small Motorized Craft (12m)' : 'Traditional Craft (<10m)' },
        cruisingSpeed
      );
      ...
      <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-4 shadow-xl">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-100 text-sm">Lower-Risk Route Planning</h2>
          </div>
      ...
      {plan && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/60 border border-rose-900/50 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1 border-t border-slate-800/80">
                ...
                <div>
                  ...
                  <span className="text-rose-400 font-medium">{direct?.hazardBreaches > 0 ? `${direct.hazardBreaches} Warning` : 'Clear'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/80 border border-cyan-800/80 space-y-2 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1 border-t border-slate-800/80">
                <div>
                  <span className="text-slate-500 block">Distance:</span>
                  <strong className="text-slate-200">{proposed?.totalDistanceNm} NM</strong> (+{proposed?.detourAdditionalNm} NM)
                </div>
                <div>
                  <span className="text-slate-500 block">Est. Time:</span>
                  <strong className="text-slate-200">{proposed?.estimatedDurationHours} Hours</strong> (+{proposed?.detourAdditionalMinutes}m)
                </div>
                <div>
                  <span className="text-slate-500 block">Max Wave:</span>
                ...
                <div>
                  <span className="text-slate-500 block">Geofence:</span>
                  <span className="text-emerald-400 font-medium">100% Clear</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {proposed?.turnByTurnDirectives?.length > 0 && (
            <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-800 space-y-2">
              <span className="font-bold text-slate-200 text-xs flex items-center gap-1.5">
                ...
                <span>Turn-by-Turn Waypoint Directives ({proposed.turnByTurnDirectives.length} Legs):</span>
              </span>
                  ...
                  >
                    <div className="space-y-0.5">
                      ...
                      <div className="text-[10px] text-slate-400">
                        Leg Distance: <strong>{leg.distanceNm} NM</strong> ({leg.distanceKm} km) &bull; Est: {leg.estimatedMinutes} mins
                      </div>
                    </div>
          ...
          <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-[10px] text-slate-500 leading-relaxed space-y-1">
            <span className="font-bold text-slate-400">Scientific Terminology & Disclaimer:</span>
            <p>
              ORCA provides <strong>"Lower-risk route recommendations"</strong> based on available satellite wave and meteorological forecasts. The platform never guarantees safety. The Vessel Master maintains sole navigational responsibility.
            </p>
          </div>
        </div>
      )}
