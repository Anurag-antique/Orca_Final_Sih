...
class RealOpenMeteoOceanProvider extends BaseProvider {
  ...
  async getOceanConditions(location, datetime = new Date()) {
    ...
    const currentSpeed = cur.ocean_current_velocity ? cur.ocean_current_velocity / 3.6 : 0.42; // convert km/h to m/s

    const oceanData = {
      location: { lat, lon },
      observationTime: cur.time ? new Date(cur.time).toISOString() : new Date().toISOString(),
      ...
      current: {
        ...
        directionCardinal: getCardinalDirection(cur.ocean_current_direction || 180),
      },
      salinityPsu: 35.6,
      seaStateCode: waveHeight > 2.0 ? 5 : waveHeight > 1.25 ? 4 : 3,
      seaStateDescription: waveHeight > 2.0 ? 'Rough sea state with large breakers' : waveHeight > 1.25 ? 'Moderate sea condition with scattered whitecaps' : 'Slight sea condition'
    };

    return this.standardizeResponse(oceanData, {
      dataset: 'Open-Meteo Marine / Copernicus Marine Environment Monitoring Service (CMEMS)',
      origin: 'European Centre for Medium-Range Weather Forecasts (ECMWF WAM & NEMO Live Model)',
