...
class FallbackOceanProvider extends BaseProvider {
  constructor() {
    super(
      "Resilient-OceanProvider-Orchestrator",
      "OCEANOGRAPHY",
      "1.0.0",
      false,
    );
  ...
  async getOceanConditions(location, datetime = new Date()) {
    try {
      ...
      const result = await this.primaryProvider.getOceanConditions(
        location,
        datetime,
      );
      return result;
      ...
      const fallbackResult = await this.fallbackProvider.getOceanConditions(
        location,
        datetime,
      );
