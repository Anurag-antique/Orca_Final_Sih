...
class RiskAssessmentEngine {
   ...
   */
  static evaluate({ weather = {}, ocean = {}, advisory = {}, geospatial = {}, vesselProfile = {} }) {
    const triggeredRules = [];

    // 1. Evaluate Individual Specialized Rules
    const windEval = WindRule.evaluate(weather.windSpeedKmh, weather.windGustsKmh);
    const waveEval = WaveRule.evaluate(ocean.significantWaveHeightM, ocean.wavePeriodSec, ocean.swellHeightM);
    const visEval = VisibilityRule.evaluate(weather.visibilityKm, weather.precipitationMm);
    const cycloneEval = CycloneAlertRule.evaluate(weather.cycloneAlert, advisory.advisories);
    ...
    triggeredRules.push(windEval, waveEval, visEval, cycloneEval, lightningEval, geofenceEval);
    ...
    const peakPrimaryHazard = Math.max(windEval.subScore, waveEval.subScore, cycloneEval.subScore);
    ...
    let finalScore = Math.round(rawCompositeScore * (vesselTypeKey === 'small_motorized' ? 1.0 : vesselMultiplier));
    ...
    if (primaryFactors.length === 0) {
      primaryFactors.push('Calm sea conditions and light breeze within safe limits');
    }

    // 7. Actionable Safety Directives
    const safetyDirectives = [];
    if (riskLevel === 'CRITICAL') {
      safetyDirectives.push('TOTAL VOYAGE BAN: Remain securely berthed in port.', 'Do not untie vessel; inspect mooring lines and hatch covers.');
    } else if (riskLevel === 'HIGH') {
      safetyDirectives.push('Small and non-mechanized craft strictly advised to abort departure.', 'Mechanized vessels must maintain continuous VHF radio watch on Channel 16.');
    } else if (riskLevel === 'MODERATE') {
      safetyDirectives.push('Exercise caution beyond 10-15 NM offshore.', 'Ensure lifejackets donned, bilge pumps operational, and flares verified.');
    } else {
      safetyDirectives.push('Normal navigation authorized within territorial waters.', 'Standard maritime safety protocols apply.');
    }
    ...
    return {
      ...
      disclaimer: 'Deterministic rule evaluation based on WMO/IMD/INCOIS criteria. Zero LLM hallucination in safety metrics.'
    };
  }
}
